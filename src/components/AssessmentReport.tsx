...
export default function AssessmentReport({
  ...
  const handleShareResult = async () => {
    ...
    const riskLabel = isRed ? '🔴 EMERGENCY — Seek Immediate Care'
      : isAmber ? '🟡 MODERATE RISK — Doctor Consultation Needed'
      : '🟢 LOW RISK — Home Care Appropriate';

    const divider = '─────────────────────────────────────';

    let msg = '';
    msg += `🏥  SYMPTACARE ASSESSMENT REPORT\n`;
    msg += `${divider}\n`;
    ...
    if (primaryRule) {
      msg += `🩺  Condition Identified:\n`;
      msg += `    ${primaryRule.metadata.description}\n\n`;
    } else if (triageResult.description) {
      msg += `🩺  Result:\n    ${triageResult.description}\n\n`;
    }

    msg += `${divider}\n`;
    msg += `💊  CLINICAL ADVICE\n`;
    if (triageResult.selfCareAdvice) {
      msg += `\n🏠  Self-Care:\n    ${triageResult.selfCareAdvice}\n`;
    }
    if (triageResult.medicationAdvice) {
      msg += `\n💊  Medication:\n    ${triageResult.medicationAdvice}\n`;
    }
    if (triageResult.escalationTrigger) {
      msg += `\n⚠️   When to Escalate:\n    ${triageResult.escalationTrigger}\n`;
    }
    if (!triageResult.selfCareAdvice && !triageResult.medicationAdvice && triageResult.triageAdvice) {
      msg += `\n    ${triageResult.triageAdvice}\n`;
    }

    const activeSymptoms = symptomList.filter(s => s.active);
    if (activeSymptoms.length > 0) {
      msg += `\n${divider}\n`;
      msg += `📝  SYMPTOMS REPORTED (${activeSymptoms.length})\n\n`;
      activeSymptoms.forEach(s => {
        const label = s.factKey.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        msg += `  •  ${label}`;
        if (s.severityCode) msg += ` — Severity: ${s.severityCode.replace(/_/g, ' ')}`;
        if (s.durationCode) msg += ` — Duration: ${s.durationCode.replace(/_/g, ' ')}`;
        msg += `\n`;
      });
    }
    ...
    if (secondaryRules.length > 0) {
      msg += `\n${divider}\n`;
      msg += `🔍  OTHER CONDITIONS MATCHED\n\n`;
      secondaryRules.forEach(r => {
        const risk = r.metadata.riskCategory === 'Red' ? '🔴' : r.metadata.riskCategory === 'Amber' ? '🟡' : '🟢';
        msg += `  ${risk}  ${r.metadata.description}\n`;
      });
    }

    msg += `\n${divider}\n`;
    msg += `⚠️   DISCLAIMER\n`;
    msg += `This report is generated by SymptaCare for informational\n`;
    msg += `purposes only. It does NOT replace professional medical\n`;
    msg += `advice, diagnosis, or treatment. Please show this report\n`;
    msg += `to a licensed physician for proper evaluation.\n`;
    msg += `${divider}\n`;
    msg += `SymptaCare v1.0 · © 2026 Group 4`;

    try {
      await Share.share({ message: msg, title: 'SymptaCare Assessment Report' });
    } catch (error) {
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          <View style={{ flexDirection: 'row', gap: SPACING.md }}>
            <Pressable
              style={[styles.startNewButton, { flex: 1, backgroundColor: isRed ? '#EF4444' : COLORS.brandNavy }]}
              onPress={onStartNew}
            >
            ...
            <Pressable
              style={[styles.startNewButton, { flex: 1, backgroundColor: isRed ? 'rgba(255,255,255,0.15)' : COLORS.bgSurface2, borderWidth: 1, borderColor: isRed ? 'rgba(255,255,255,0.25)' : COLORS.borderLight }]}
              onPress={handleShareResult}
            >
              <Feather name="share-2" size={16} color={isRed ? 'white' : COLORS.brandNavy} style={{ marginRight: SPACING.sm }} />
              <Text style={[styles.startNewButtonText, { color: isRed ? 'white' : COLORS.brandNavy }]}>Share Result</Text>
            </Pressable>
          </View>
        </View>
